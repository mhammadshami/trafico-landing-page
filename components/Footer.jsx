...
const Footer = ({ footerData }) => {
  ...
  return (
    <footer className="bg-darkblue lg:bg-transparent lg:bg-footer lg:bg-no-repeat lg:bg-left-bottom min-h-[738px] pt-12 lg:pt-0 relative">
      <motion.div
      ...
      >
        ...
        >
          {/* text */}
          <div className="text-white flex-1 border-l-[10px] border-accent">
          ...
          <form className="w-full max-w-[550px] bg-white flex-1 rounded-[10px] lg:order-2 flex flex-col px-[35px] lg:px-[75px] py-[25px] lg:py-[52px] space-y-[40px] drop-shadow-primary ">
            ...
            <div className="flex flex-col">
              ...
              />
              {/* button */}
              <button className="btn self-start hover:bg-accent-hover transition mt-6">
                {form.btnText}
                {/* icon */}
        ...
        >
          ...
          {/* links */}
          <ul className="flex flex-col items-center lg:flex-row gap-y-4 lg:gap-y-0 lg:gap-x-[100px] text-center">
            {links.map((item, idx) => {
              return (
                <li
